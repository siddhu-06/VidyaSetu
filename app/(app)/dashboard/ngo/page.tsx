    ...
    <div className="min-h-screen px-4 py-8 md:px-8" style={{ background: '#0C0F0A' }}>
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold" style={{ color: '#B8F04A' }}>
              NGO Admin Dashboard
            </h1>
            <p className="mt-2 text-sm" style={{ color: '#7A8872' }}>
              Welcome back {profile?.first_name ?? 'Admin'} · {profile?.org_name || 'Organisation'}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {['Students', 'Volunteers', 'CSR Report', 'Analytics'].map((section) => (
            <section
              key={section}
